...
class NewProperty extends HookWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => getIt<PropertyFormBloc>()..add(
        PropertyFormEvent.initialized(dz.none())
      ),
      child: Scaffold(
...
class CreatePropertyFormWidget extends HookWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<PropertyFormBloc, PropertyFormState>(
      listenWhen: (p,c ) => p.propertyFailureOrSuccessOption != c.propertyFailureOrSuccessOption,
      listener: (context, state) {
          ...
          state.propertyFailureOrSuccessOption.fold(
                ...
                .fold(
                    (failure) => Flushbar(
                  message: failure.map(
                      serverError: (_) => "Sorry we were unable to upload the property.",
                      unExpected: (_) => "An error occurred please try again later.",
                      uploadFileFailed: (_) => "Image upload failed",
                      insufficientPermission: (_) => "You do not have permission to create the property.",
                      unableToUpdate: (_) => "",
                      wrongFormat: (_) => "",
                      emptyDocuments: (_) => "",
                  ),
      ...
      builder: (context, state) {
        return Form(
          key: formKey,
          autovalidateMode: state.showErrors ? AutovalidateMode.always
            ...
            children: [
              ...
              InkWell(
                      ...
                      builder: (context) => SizedBox(
                        child: Column(
                          ...
                          children: [
                            ListTile(
                              onTap: () async {
                                final Position geo = await _getLocation(context);
                                final geoPoint = GeoPoint(geo.latitude, geo.longitude);

                                outContext
                                .add(PropertyFormEvent.locationChanged(geoPoint));
                                Navigator.of(context).pop();
                              },
                              title: const Text("Current location"),
                              subtitle: const Text("Set my current location as default property location")
                            ),
                            ListTile(
                                onTap: () async {
                                  //
                                  ...
                                  final markerIcon = await getBytesFromAsset('assets/images/destination_marker.png', 70);
                                  pinLocationIcon.value = BitmapDescriptor.fromBytes(markerIcon);
                                      ...
                                      onDragEnd: (loc) {
                                        showDialog(
                                          ...
                                          builder: (context) => AlertDialog(
                                            ...
                                            content: const Text("Set current picker location as property location?"),
                                ...
                                subtitle: Text("Drag the marker to property location.")
                            )
                          ]
                        )
                      )
                ...
                child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: state.hasLocation ? Colors.white.withOpacity(0.75) : "#E5F0FF".toColor(),
                      borderRadius: BorderRadius.circular(8.0),
                    ),
                    child: Column(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          if (state.hasLocation) Icon(
                            EvaIcons.bulbOutline,
                            size: 50,
                            color: "#00DFC8".toColor()
                          ) else const Icon(
                              EvaIcons.pinOutline,
                              size: 50
                          ),
                          const SizedBox(height: 10),
                          Text(
                              state.hasLocation ? "Location Successfully Set" : "Get location", style: TextStyle(color: Colors.black54, fontFamily: "ProductSans")),
                        ]
                    )
                ),
              ),
              ...
              TextFormField(
                ...
                onTap: () async {
                  ...
                  context.read<PropertyFormBloc>().add(PropertyFormEvent.dateChanged(date.toLocal()));
                },
              ...
              Padding(
                ...
                child: RaisedButton(
                    ...
                    child: state.isSaving ? const SpinKitRing(
                        color: Colors.white,
                        lineWidth: 2.0,
                        size: 16
                    ) : const Text(
